<template>
  <div class="login">
    ...
    <div class="login__content">
      <div class="login__subtitle">Авторизация</div>

      <form @submit.prevent="onSubmit" class="login__form">
        <div class="login__form-item">
          <FormItem name="email" type="text" label="Email" placeholder="test@test.com" :disabled="isSubmit" />
        </div>
        <div class="login__form-item">
          <FormItem name="password" type="password" label="Пароль" :disabled="isSubmit" />
        </div>

        <div v-if="ErrorSubmit" class="login__form-error">{{ ErrorSubmit }}</div>

        <div class="login__form-submit">
          <button :disabled="isSubmit" class="btn btn--primary">Войти</button>
          <Icons v-if="isSubmit" name="loader" />
        </div>
      </form>

      <router-link to="/signup" class="login__redirect">Зарегистрироваться</router-link>
    </div>
  </div>
</template>
